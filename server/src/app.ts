...
import {
  Room, Player, ServerToClientEvents, ClientToServerEvents,
  PlayerInfo, POWER_UPS,
} from './types';
import {
  getQuestions, getAvailableCategories, getMiniGames,
  getHostComment, getRandomFunFact,
} from './questions';
import {
  generateRoomId, getBasePoints, getSpeedBonus,
  computeCategoryVoteResult, computeRoundScore,
  computePyramidStartPositions, computeFinalScores,
} from './utils';
...
export function createApp(allowedOrigins: string[] = ['http://localhost:5173']) {
  const app = express();
  ...
  function startCategoryVote(room: Room) {
    ...
    const tickInterval = setInterval(() => {
      ...
      if (timeLeft <= 0) { clearInterval(tickInterval); resolveCategoryVote(room); }
    }, 1000);
  ...
  function resolveCategoryVote(room: Room) {
    clearTimer(room);
    const result = computeCategoryVoteResult(room.players, room.availableCategories);

    room.selectedCategory = result.selectedCategory;
    if (result.overridePlayerId) {
      const op = room.players.find((p) => p.id === result.overridePlayerId);
      if (op) { op.usedOverride = true; op.hasOverride = false; }
    }
    ...
    room.timer = setTimeout(() => { startPowerUpPhase(room); }, 3000);
  }
  ...
  function startPowerUpPhase(room: Room) {
    ...
    room.players.forEach((p) => {
      const opponents = room.players.filter((o) => o.id !== p.id).map((o) => ({ id: o.id, name: o.name }));
      io.to(p.id).emit('game:power-up-phase', { availablePowerUps: POWER_UPS, opponents, timeLimit: 8 });
    });
    let timeLeft = 8;
    const tickInterval = setInterval(() => {
      timeLeft--;
      io.to(room.id).emit('game:tick', timeLeft);
      if (timeLeft <= 0) { clearInterval(tickInterval); startQuestion(room); }
    }, 1000);
    room.timer = tickInterval;
  }
  ...
  function startQuestion(room: Room) {
    ...
    room.players.forEach((p) => { p.currentAnswer = null; p.answeredAt = null; p.answered = false; });
    ...
    setTimeout(() => {
      ...
      room.players.forEach((player) => {
        ...
        if (!entry) return;
        if (entry.type === 'double' || entry.type === 'fifty') {
          if (entry.type === 'fifty') {
      ...
      room.players.forEach((target) => {
        const attacks = Object.entries(room.currentPowerUps)
          .filter(([, data]) => data && data.targetId === target.id && data.type !== 'double' && data.type !== 'fifty')
          .map(([attackerId, data]) => ({ attackerId, type: data!.type }));
        if (attacks.length > 0) {
          const attacker = room.players.find((pl) => pl.id === attacks[0].attackerId);
          io.to(target.id).emit('game:power-up-hit', { type: attacks[0].type, fromPlayerName: attacker?.name || '?' });
          sendHostComment(room, 'powerUpUsed');
        }
      });
    }, 1000);
    ...
    const tickInterval = setInterval(() => {
      timeLeft--;
      io.to(room.id).emit('game:tick', timeLeft);
      if (timeLeft <= 0) { clearInterval(tickInterval); revealAnswer(room); }
    }, 1000);
    room.timer = tickInterval;
  }
  ...
  function revealAnswer(room: Room) {
    ...
    const playerResults = room.players.map((p) => {
      const powerUp = room.currentPowerUps[p.id];
      const { pointsEarned, speedBonusGiven: sb } = computeRoundScore(p, question, powerUp, questionStartTime, room.roundTime);
      if (sb) speedBonusGiven = true;
      p.score += pointsEarned;
      const answerTime = p.answeredAt !== null ? p.answeredAt - questionStartTime : null;
      return { id: p.id, name: p.name, avatarId: p.avatarId, score: p.score, pointsEarned, answer: p.currentAnswer, answerTime };
    });
    ...
    io.to(room.id).emit('game:reveal', { correctIndex: question.correctIndex, players: playerResults, speedBonus: speedBonusGiven });
    room.currentQuestionIndex++;
    room.roundInGroup++;
    room.timer = setTimeout(() => { advanceGameFlow(room); }, 4000);
  }

  function advanceGameFlow(room: Room) {
    if (room.roundInGroup >= QUESTIONS_PER_GROUP) {
      room.roundInGroup = 0;
      if (room.roundGroup < TOTAL_GROUPS - 1) { startMiniGame(room); return; }
  ...
  function startMiniGame(room: Room) {
    ...
    io.to(room.id).emit('game:minigame-start', { gameNumber: room.currentMiniGame + 1, game: room.miniGames[room.currentMiniGame], timeLimit: 30 });
    let timeLeft = 30;
    const tickInterval = setInterval(() => {
      timeLeft--;
      io.to(room.id).emit('game:tick', timeLeft);
      if (timeLeft <= 0) { clearInterval(tickInterval); endMiniGame(room); }
    }, 1000);
    room.timer = tickInterval;
  }
  ...
  function endMiniGame(room: Room) {
    ...
    room.players.forEach((p) => { p.score += p.miniGameScore * 5; });
    io.to(room.id).emit('game:minigame-results', {
      players: room.players.map((p) => ({ id: p.id, name: p.name, avatarId: p.avatarId, score: p.score, miniGameScore: p.miniGameScore })),
    });
    room.currentMiniGame++;
    room.roundGroup++;
    room.timer = setTimeout(() => { startCategoryVote(room); }, 5000);
  }
  ...
  function startPyramidIntro(room: Room) {
    ...
    const { positions, isCloseGame } = computePyramidStartPositions(room.players);
    room.players.forEach((p) => { p.pyramidPosition = positions[p.id] ?? 0; });
    if (isCloseGame) sendHostComment(room, 'closeGame');

    io.to(room.id).emit('game:pyramid-intro', {
      players: room.players.map((p) => ({ id: p.id, name: p.name, avatarId: p.avatarId, score: p.score, startPosition: p.pyramidPosition })),
      pyramidSize: room.pyramidSize,
    });
    room.timer = setTimeout(() => { nextPyramidQuestion(room); }, 5000);
  }

  function nextPyramidQuestion(room: Room) {
    const winner = room.players.find((p) => p.pyramidPosition >= room.pyramidSize);
    if (winner || room.pyramidQuestionIndex >= room.pyramidQuestions.length) { endGame(room); return; }
    ...
    io.to(room.id).emit('game:pyramid-question', {
      question: question.question, answers: question.answers, category: question.category, timeLimit: 10,
      positions: Object.fromEntries(room.players.map((p) => [p.id, p.pyramidPosition])),
      pyramidSize: room.pyramidSize, imageUrl: question.imageUrl,
    });
    ...
    const tickInterval = setInterval(() => {
      timeLeft--;
      io.to(room.id).emit('game:tick', timeLeft);
      if (timeLeft <= 0) { clearInterval(tickInterval); revealPyramidAnswer(room); }
    }, 1000);
    room.timer = tickInterval;
  }
  ...
  function revealPyramidAnswer(room: Room) {
    ...
    const results = room.players.map((p) => {
      ...
      return { id: p.id, name: p.name, avatarId: p.avatarId, answer: p.currentAnswer, correct, newPosition: p.pyramidPosition };
    });
    io.to(room.id).emit('game:pyramid-reveal', { correctIndex: question.correctIndex, players: results, pyramidSize: room.pyramidSize });
    room.pyramidQuestionIndex++;
    room.timer = setTimeout(() => { nextPyramidQuestion(room); }, 3000);
  }

  function endGame(room: Room) {
    clearTimer(room);
    room.phase = 'finished';
    const result = computeFinalScores(room.players, room.pyramidSize);
    // Apply pyramid bonus to room player scores for consistency
    room.players.forEach((p) => { p.score += p.pyramidPosition * 20; });

    io.to(room.id).emit('game:finished', {
      players: result.sorted.map((s) => {
        const p = room.players.find((rp) => rp.id === s.id)!;
        return { id: p.id, name: p.name, avatarId: p.avatarId, score: s.score };
      }),
